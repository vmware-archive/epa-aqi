def generate_measure_of_category(number, name)
  "{
    \"Category\": {
      \"Number\": #{number},
      \"Name\": \"#{name}\"
...
def generate_measures_of_category(number, name)
  JSON.parse('[' + generate_measure_of_category(number, name)+']')
end
  ...
  describe '#for_zipcode' do
    ...
    describe 'when the least healthy measure is not the first' do
      let(:expected_measures) {
        JSON.parse('[' +
                       generate_measure_of_category(1, 'Good') + ', ' +
                       generate_measure_of_category(6, 'Hazardous') +
                   ']');
      }
      specify { expect(subject).to eq 'F'}
    end
